 ...
 * 2、预授权完成接口（OrderPreAuthCompleteTest）
 * 3、预授权撤销（OrderQuashPreAuthTest）
 * 4、预授权完成撤销（OrderQuashPreAuthCompleteTest）
 * 5、订单退款申请接口（OrderRefundReqTest）
 * 6、订单退款查询接口（OrderRefundQueryTest）
 */
...
require_once IA_ROOT . '/addons/xuan_mixloan/lib/yilian/src/com/payeco/client/TransactionClientPreAuth.php';
...
class DemoTest {
	...
	static function OrderQueryTest($orderId) {
		...
		try {
			...
			$ret = TransactionClientPreAuth::OrderQuery($merchantId, $merchOrderId, $tradeTime,
	 ...
	 * 订单预授权完成接口测试
	 */
	static function OrderPreAuthCompleteTest($merDealId, $orderId, $amt) {
		// 设置参数
		$amount = $amt;
		$merchantId = Constants::getMerchantId();
		$merchOrderId = $orderId;  //需要填写已经存在的商户订单号
		...
		// 调用预授权完成接口
		Log::logFile("-------订单预授权完成接口测试-------------------------");
		Log::logFile("orderId=".$orderId);
		try {
			$retXml = new Xml();
			//接口参数请参考TransactionClient的参数说明
			$ret = TransactionClientPreAuth::preAuthComplete($merchantId,$merDealId, $merchOrderId, $amount, $tradeTime, 
					Constants::getMerchantRsaPrivateKey(), Constants::getPayecoRsaPublicKey(),
					Constants::getPayecoUrl(), $retXml);
			if(strcmp("Y100", $ret)){
				Log::logFile("商户预授权完成接口测试失败！：retCode=".$ret."; msg=".$retXml->getRetMsg());
				return;
			}
		} catch (Exception $e) {
			Log::logFile("订单预授权完成接口测试失败！：".$e->getMessage());
			return;
		}
		Log::logFile("订单预授权完成接口测试----ok");
		Log::logFile("------------------------------------------------");
	}
	...
	static function OrderPreAuthCompleteTest($merDealId, $orderId, $amt) {
		...
		try {
			$retXml = new Xml();
			//接口参数请参考TransactionClient的参数说明
			$ret = TransactionClientPreAuth::OrderReversePreAuth($merchantId,$merDealId, $merchOrderId, $amount, $tradeTime, $tradeCode,
					Constants::getMerchantRsaPrivateKey(), Constants::getPayecoRsaPublicKey(),
					Constants::getPayecoUrl(), $retXml);
			if(strcmp("Y100", $ret)){
				Log::logFile("商户预授权撤销接口测试失败！：retCode=".$ret."; msg=".$retXml->getRetMsg());
				return;
			}
		} catch (Exception $e) {
			Log::logFile("订单预授权撤销接口测试失败！：".$e->getMessage());
			return;
		}
		Log::logFile("订单预授权撤销接口测试----ok");
		Log::logFile("------------------------------------------------");
	}
	...
	static function OrderPreAuthCompleteTest($merDealId, $orderId, $amt) {
		...
		$merchOrderId = $orderId;  //需要填写已经存在的商户订单号
		$tradeTime = Tools::getSysTime();
		$tradeCode = "QuashPreAuthComplete";

		// 调用订单预授权完成撤消接口
		Log::logFile("-------订单预授权完成撤消接口测试-------------------------");
		Log::logFile("orderId=".$orderId);
		try {
			$retXml = new Xml();
			//接口参数请参考TransactionClient的参数说明
			$ret = TransactionClientPreAuth::OrderReversePreAuth($merchantId,$merDealId, $merchOrderId, $amount, $tradeTime, $tradeCode,
					...
					Constants::getPayecoUrl(), $retXml);
			if(strcmp("Y100", $ret)){
				Log::logFile("商户预授权完成撤消接口测试失败！：retCode=".$ret."; msg=".$retXml->getRetMsg());
				return;
			}
		} catch (Exception $e) {
			Log::logFile("订单预授权完成撤消接口测试失败！：".$e->getMessage());
			return;
		}
		Log::logFile("订单预授权完成撤消接口测试----ok");
		Log::logFile("------------------------------------------------");
	}
