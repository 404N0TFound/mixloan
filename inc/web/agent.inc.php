    ...
    if ($_GPC['post'] == 1) {
        ...
        if ($_GPC['data']['status'] == 1 && $re_money>0) {
            if ($item['degree'] == 1) {
                ...
                $nickname = pdo_fetchcolumn("select b.nickname from ".tablename('xuan_mixloan_product_apply').' a left join '.tablename('xuan_mixloan_member')." b ON a.inviter=b.id WHERE a.pid={$item['pid']} AND a.uid={$item['uid']} AND a.degree=1");
                $value = "您好，您的徒弟{$nickname}邀请了{$item['realname']}成功注册了{$info['name']}，奖励您推广佣金，继续推荐产品，即可获得更多佣金奖励";
            }
            $datam = array(
                "first" => array(
                    "value" => $value,
