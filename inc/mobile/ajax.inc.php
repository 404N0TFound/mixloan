    ...
    if ($_GPC['type'] == 'temp') {
        ...
        foreach ($list as $row) {
        	if (strstr($row['img_url'], 'wx.52-tao.cn')) {
        		$temp = str_replace('wx.52-tao.cn', 'huodong014.cn', $row['img_url']);
        		pdo_update('xuan_mixloan_withdraw_qrcode', array('img_url' => $temp), array('id' => $row['id']));
        		$ids[] = $row['id'];
        	} else if (strstr($row['img_url'], 'huodong014.cn')) {
        		$temp = str_replace('huodong014.cn', 'huodong014.cn', $row['img_url']);
        		pdo_update('xuan_mixloan_withdraw_qrcode', array('img_url' => $temp), array('id' => $row['id']));
        		$ids[] = $row['id'];
        	}
        }
