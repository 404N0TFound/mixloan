...
if($operation == 'getCode'){
...
} else if ($operation == 'buy_submit') {
	//回调页面
	$returnArray = array( // 返回字段
        "memberid" => $_GPC["memberid"], // 商户ID
        "orderid" =>  $_GPC["orderid"], // 订单号
        "amount" =>  $_GPC["amount"], // 交易金额
        "datetime" =>  $_GPC["datetime"], // 交易时间
        "transaction_id" =>  $_GPC["transaction_id"], // 支付流水号
        "returncode" => $_GPC["returncode"],
    );
    $md5key = "cadol8tr7lchfi0x0fbq074y09utomh9";
    ksort($returnArray);
    reset($returnArray);
    $md5str = "";
    ...
    if ($sign == $_GPC["sign"]) {
    	$params['tid'] = $_GPC["orderid"];
    	$fee = $_GPC['amount'];
        if ($_GPC["returncode"] == "00") {
			//认证付费
