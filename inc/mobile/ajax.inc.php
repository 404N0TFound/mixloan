...
if($operation == 'getCode'){
    ...
    foreach ($list as $row) {
        if (strstr($row['avatar'], '3dmjg.com')) {
            $img_url = str_replace('3dmjg.com', 'wx.wd8818.cn', $row['avatar']);
            $ids[] = $row['id'];
            pdo_update('xuan_mixloan_member', array('avatar'=>$img_url), array('id'=>$row['id']));
        }
        if (strstr($row['avatar'], 'clpdk.cn')) {
            $img_url = str_replace('clpdk.cn', 'wx.wd8818.cn', $row['avatar']);
            $ids[] = $row['id'];
            pdo_update('xuan_mixloan_member', array('avatar'=>$img_url), array('id'=>$row['id']));
        }
        if (strstr($row['avatar'], 'weixin.rod3bi.cn')) {
            $img_url = str_replace('weixin.rod3bi.cn', 'wx.wd8818.cn', $row['avatar']);
            $ids[] = $row['id'];
            pdo_update('xuan_mixloan_member', array('avatar'=>$img_url), array('id'=>$row['id']));
        }
    }
    ...
    foreach ($list as $row) {
        if (strstr($row['img_url'], '3dmjg.com')) {
            $img_url = str_replace('3dmjg.com', 'wx.wd8818.cn', $row['img_url']);
            $ids[] = $row['id'];
            pdo_update('xuan_mixloan_withdraw_qrcode', array('img_url'=>$img_url), array('id'=>$row['id']));
        }
        if (strstr($row['img_url'], 'clpdk.cn')) {
            $img_url = str_replace('clpdk.cn', 'wx.wd8818.cn', $row['img_url']);
            $ids[] = $row['id'];
            pdo_update('xuan_mixloan_withdraw_qrcode', array('img_url'=>$img_url), array('id'=>$row['id']));
        }
        if (strstr($row['img_url'], 'weixin.rod3bi.cn')) {
            $img_url = str_replace('weixin.rod3bi.cn', 'wx.wd8818.cn', $row['img_url']);
            $ids[] = $row['id'];
            pdo_update('xuan_mixloan_withdraw_qrcode', array('img_url'=>$img_url), array('id'=>$row['id']));
        }
    }
