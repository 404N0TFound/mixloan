...
} else if ($operation == 'yun_pay') {
	//云支付异步通知
	$openid = m('user')->getOpenid();
	$member = m('member')->getInfo($openid);
	require_once '../addons/xuan_mixloan/lib/shanpayfunction.php';
	$out_order_no = $_REQUEST['out_order_no'];
	$total_fee = $_REQUEST['total_fee'];
	$trade_status = $_REQUEST['trade_status'];
	$sign = $_REQUEST['sign'];
	$shanNotify = md5VerifyShan($out_order_no,$total_fee,$trade_status,$sign,$config['yunpay_md5key'],$config['yunpay_partner']);
	if($shanNotify) {
		if($trade_status=='TRADE_SUCCESS'){
			...
			message("交易失败",$this->createMobileUrl('vip', ['op'=>'buy']),"error");
		}
	} else {
		message("交易失败",$this->createMobileUrl('vip', ['op'=>'buy']),"error");
	}
