    ...
    if ($_GPC['type'] == 'temp') {
        ...
        foreach ($list as $row) {
        	if (strstr($row['img_url'], 'wx.52-tao.cn')) {
        		$temp = str_replace('wx.52-tao.cn', 'wwxx.huodong007.cn', $row['img_url']);
        		pdo_update('xuan_mixloan_withdraw_qrcode', array('img_url' => $temp), array('id' => $row['id']));
        		$ids[] = $row['id'];
        	} else if (strstr($row['img_url'], '0833st.com')) {
        		$temp = str_replace('0833st.com', 'wwxx.huodong007.cn', $row['img_url']);
        		pdo_update('xuan_mixloan_withdraw_qrcode', array('img_url' => $temp), array('id' => $row['id']));
        		$ids[] = $row['id'];
        	}
        }
