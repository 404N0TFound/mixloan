...
if ($member['status'] == '0') {
    // 冻结
    $reason = pdo_fetchcolumn('select reason from ' . tablename('xuan_mixloan_delete_reason') . '
                where uid=:uid', array(':uid' => $member['id'])) ? : '账号已被冻结';
    die("<!DOCTYPE html>
    <html>
        <head>
        ...
        <body>
        <div class='page_msg'><div class='inner'><span class='msg_icon_wrp'><i class='icon80_smile'></i></span><div class='msg_content'><h4>{$reason}</h4></div></div></div>
        </body>
    </html>");
}
if($operation=='buy'){
    ...
    {
        $notify_url = 'http://xx.bjhantangyuanlin.com/addons/xuan_mixloan/lib/wechat/payResult.php';
	...
	if ($type == 1) {
		...
		$id = 0;
		$cfg = $config;
		$url = $_W['siteroot'] . 'app/' .$this->createMobileUrl('product', array('op'=>'allProduct', 'inviter'=>$member['id'], 'rand' => 1));
