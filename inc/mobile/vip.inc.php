...
if($operation=='buy'){
    ...
    {
        $notify_url = 'http://wq.xiushenghuo.com.cn/addons/xuan_mixloan/lib/wechat/payResult.php';
        ...
        if ($result['code'] == 1) {
            $redirect_url = urlencode( 'http://wq.xiushenghuo.com.cn/app/' .
	...
	if ($type == 1) {
		...
		if ($product['type'] == 1){
            $url =  'http://wq.xiushenghuo.com.cn/app/' .$this->createMobileUrl('product', array('op'=>'apply', 'id'=>$id, 'inviter'=>$member['id']));
        } else {
            $url = 'http://wq.xiushenghuo.com.cn/app/' .$this->createMobileUrl('loan', array('op'=>'apply', 'id'=>$product['relate_id'], 'pid'=>$id, 'inviter'=>$member['id']));
        }
		...
		$id = 0;
		$cfg = $config;
		$url = 'http://wq.xiushenghuo.com.cn/app/' .$this->createMobileUrl('product', array('op'=>'allProduct', 'inviter'=>$member['id']));
    ...
    if ($config['wx_invite_code']) {
        ...
        $url = 'http://wq.xiushenghuo.com.cn/app/' .$this->createMobileUrl('vip', array('op' => 'app_register','inviter'=>$member['id']));
    }
