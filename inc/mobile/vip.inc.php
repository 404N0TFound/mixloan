...
if($operation=='buy'){
	...
	if ($type == 1) {
		...
		if ($product['type'] == 1){
            $url =  'http://day009.com/app/' .$this->createMobileUrl('product', array('op'=>'apply', 'id'=>$id, 'inviter'=>$member['id']));
        } else {
            $url = 'http://day009.com/app/' .$this->createMobileUrl('loan', array('op'=>'apply', 'id'=>$product['relate_id'], 'pid'=>$id, 'inviter'=>$member['id']));
        }
		...
		$id = 0;
		$cfg = $config;
		$url = 'http://day009.com/app/' .$this->createMobileUrl('product', array('op'=>'allProduct', 'inviter'=>$member['id']));
    ...
    if ($config['wx_invite_code']) {
        ...
        $url = 'http://day009.com/app/' .$this->createMobileUrl('vip', array('op' => 'app_register','inviter'=>$member['id']));
    }
