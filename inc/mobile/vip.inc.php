...
if($operation=='buy'){
	...
	unset($row);
	$accounts_list = pdo_fetchall("SELECT a.id,a.bonus,a.createtime,b.name FROM ".tablename("xuan_mixloan_withdraw")." a LEFT JOIN ".tablename("xuan_mixloan_withdraw_qrcode")." b ON a.bank_id=b.id WHERE a.uid={$member['id']} ORDER BY id DESC");
    ...
    $buy_count = pdo_fetchcolumn("SELECT count(*) FROM ".tablename("xuan_mixloan_product_apply")." a LEFT JOIN ".tablename("xuan_mixloan_member"). " b ON a.uid=b.id WHERE a.inviter={$member['id']} AND a.status>0 AND pid=0 ORDER BY a.id DESC");
