...
if($operation=='buy'){
    ...
    {
        $notify_url = 'http://xz.zhongchuangkm.com/addons/xuan_mixloan/lib/wechat/payResult.php';
	...
	//提现
    $banks = pdo_fetchall("SELECT * FROM ".tablename("xuan_mixloan_creditCard")." WHERE uid=:uid and status=1 and type=2", array(':uid'=>$member['id']));
    ...
    if ($type == 3) {
        ...
        $tips = "HI，朋友，为你介绍一款赚钱神器，推荐他人办卡办贷，日日领工资，邀你一起体验:{$short_url}";
        if (!$posterArr) {
