...
if($operation=='buy'){
    ...
    {
        $notify_url = 'http://www.hulianwang114.com/addons/xuan_mixloan/lib/wechat/payResult.php';
    ...
    if ($type=='10001') {
        ...
        $url = 'http://banma.tainchangshi.top/app/' .$this->createMobileUrl('vip', array('op'=>'salary'));
	...
	if ($type == 1) {
		...
		if ($product['type'] == 1){
            $url = 'http://banma.tainchangshi.top/app/' .$this->createMobileUrl('product', array('op'=>'apply', 'id'=>$id, 'inviter'=>$member['id']));
        } else {
            $url = 'http://banma.tainchangshi.top/app/' .$this->createMobileUrl('loan', array('op'=>'apply', 'id'=>$product['relate_id'], 'pid'=>$id, 'inviter'=>$member['id']));
        }
		...
		$id = 0;
		$cfg = $config;
		$url = 'http://banma.tainchangshi.top/app/' .$this->createMobileUrl('product', array('op'=>'allProduct', 'inviter'=>$member['id']));
