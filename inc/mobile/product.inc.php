...
if($operation=='index'){
    ...
    if ($info['type'] == 1) {
        $poster_short_url = shortUrl('http://wwx.uo111.cn/app/' .$this->createMobileUrl('product', array('op'=>'apply', 'id'=>$id, 'inviter'=>$member['id'], 'rand'=>1)));
        $poster_long_url = 'http://wwx.uo111.cn/app/' .$this->createMobileUrl('product', array('op'=>'apply', 'id'=>$id, 'inviter'=>$member['id'], 'rand'=>1));
    } else {
