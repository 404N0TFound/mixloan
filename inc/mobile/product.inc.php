...
if($operation=='index'){
    //首页
    $new_list = m('product')->getList([], ['is_show'=>1], ' id desc', 21);
    ...
    if ($info['type'] == 1) {
        ...
        $poster_long_url = $_W['siteroot'] . 'app/' .$this->createMobileUrl('product', array('op'=>'apply', 'id'=>$id, 'inviter'=>$member['id'], 'rand'=>2));
    } else {
        ...
        $poster_long_url = $_W['siteroot'] . 'app/' .$this->createMobileUrl('loan', array('op'=>'apply', 'id'=>$info['relate_id'], 'inviter'=>$member['id'], 'pid'=>$info['id'], 'rand'=>2));
    }
    ...
    if ($_COOKIE['apply']) {
        show_json(-1, [], "您的申请过于频繁");
    } else {
        setcookie('apply', 1, time()+10);
    }
