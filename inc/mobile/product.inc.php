...
if($operation=='index'){
    ...
    if ($info['type'] == 1) {
        $poster_short_url = shortUrl('http://wwx.uo111.cn/app/' .$this->createMobileUrl('product', array('op'=>'apply', 'id'=>$id, 'inviter'=>$member['id'])));
        $poster_long_url = 'http://wwx.uo111.cn/app/' .$this->createMobileUrl('product', array('op'=>'apply', 'id'=>$id, 'inviter'=>$member['id']));
    } else {
        $poster_short_url = shortUrl('http://wwx.uo111.cn/app/' .$this->createMobileUrl('loan', array('op'=>'apply', 'id'=>$info['relate_id'], 'inviter'=>$member['id'], 'pid'=>$info['id'])));
        $poster_long_url = 'http://wwx.uo111.cn/app/' .$this->createMobileUrl('loan', array('op'=>'apply', 'id'=>$info['relate_id'], 'inviter'=>$member['id'], 'pid'=>$info['id']));
    }
