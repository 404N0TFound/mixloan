...
if($operation=='index'){
    ...
    if ($info['type'] == 1) {
        $poster_short_url = shortUrl( 'http://wx.wangi.cn/app/' .$this->createMobileUrl('product', array('op'=>'apply', 'id'=>$id, 'inviter'=>$member['id'], 'rand'=>2)));
        $poster_long_url =  'http://wx.wangi.cn/app/' .$this->createMobileUrl('product', array('op'=>'apply', 'id'=>$id, 'inviter'=>$member['id'], 'rand'=>time()));
    } else {
        $poster_short_url = shortUrl( 'http://wx.wangi.cn/app/' .$this->createMobileUrl('loan', array('op'=>'apply', 'id'=>$info['relate_id'], 'inviter'=>$member['id'], 'pid'=>$info['id'], 'rand'=>2)) );
        $poster_long_url =  'http://wx.wangi.cn/app/' .$this->createMobileUrl('loan', array('op'=>'apply', 'id'=>$info['relate_id'], 'inviter'=>$member['id'], 'pid'=>$info['id'], 'rand'=>time()));
    }
