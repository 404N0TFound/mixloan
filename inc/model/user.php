...
class Xuan_mixloan_User
{
    ...
    function getOpenid($uid="")
    {
        ...
        $uid = intval($uid);
        if ($uid) {
            $openid = pdo_fetchcolumn("select openid from ".tablename("xuan_mixloan_member")." where id={$uid}");
        } else {
    ...
    function getPerOpenid()
    {
        ...
        @session_start();
        $cookieid = "__cookie_xuan_mixloan_openid_{$_W['uniacid']}";
    ...
    function oauth_info()
    {
        ...
        session_set_cookie_params($lifeTime);
        @session_start();
        $sessionid = "__cookie_xuan_mixloan_201507100000_{$_W['uniacid']}";
