    ...
    **/
    ...
    public function createNewPoster($params) {
        ...
        if (!empty($ext_info['poster']['nickname'])) {
            ...
            $top = intval($ext_info['poster']['nickname']['top'])* ($width_proportion+0.1);
            $color = imagecolorallocatealpha($bgpng,$poster_color['r'],$poster_color['g'],$poster_color['b'],0);
            $result = imagettftext($bgpng,$ext_info['poster']['nickname']['size']*$width_proportion,0,$left,$top,$color,$font,$params['member']['nickname']);
        }
        $res = imagepng($bgpng,$params['out']);
        @imagedestroy($QR);
        imagedestroy($bgpng);
