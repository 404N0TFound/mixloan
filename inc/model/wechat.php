...
class Xuan_mixloan_Wechat
{
    ...
    {
        global $_W;
        $this->d = pdo_fetch("select payment from ".tablename('uni_settings')."where uniacid ={$_W['uniacid']}");
        $this->result = unserialize($this->d['payment']);
        $this->config = pdo_fetch("select * from ".tablename('account_wechats')." where uniacid={$_W['uniacid']}");
        $this->options = array (
            'token' => $this->config ["token"], // 填写你设定的key
            'encodingaeskey' => $this->config ["encodingaeskey"], // 填写加密用的EncodingAESKey
            'appid' => $this->config ["key"], // 填写高级调用功能的app id
            'appsecret' => $this->config ["secret"], // 填写高级调用功能的密钥
            'partnerid' => $this->result['wechat']['mchid'], // 商户ID
            'partnerkey' =>  $this->result['wechat']['apikey'], // 商户密钥密钥
            //'paysignkey' => ''  //老版本商户号，新版本已废弃
        );
        ...
        $this->partnerid = isset($this->options['partnerid'])?$this->options['partnerid']:'';
        $this->partnerkey = isset($this->options['partnerkey'])?$this->options['partnerkey']:'';
        $this->paysignkey = isset($this->options['paysignkey'])?$this->options['paysignkey']:'';
        $this->debug = isset($this->options['debug'])?$this->options['debug']:false;
        $this->_logcallback = isset($this->options['logcallback'])?$this->options['logcallback']:false;
    }
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    /*  public function index_dddd(){
          $pkc_encoder = new PKCS7Encoder;
           $pkc_encoder->index_dddd();
           echo 54564;
      }*/
    //向用户发送文本提醒信息
    public function sendtxtmsg($wechatid,$msg){
        $data['touser'] =  $wechatid;
        $data['msgtype'] = 'text';
        $data['text']['content'] = $msg;
        $this->sendCustomMessage($data);

    }
    public function sxtmsg($wechatid,$msg){
        $data['touser'] =  $wechatid;
        $data['msgtype'] = 'text';
        $data['text']['content'] = $msg;
        $this->sendCustomMessage($data);
    ...
    /**
     * 获取jsapi_ticket用户微信JSSDK
     */
    public function check_jsapi_ticket($appid='',$appsecret='',$token=''){
        if (!$this->access_token && !$this->checkAuth()) return false;
        ...
        $result = $this->http_get('https://api.weixin.qq.com/cgi-bin/ticket/getticket?access_token='.$access_token.'&type=jsapi');
        if ($result)
        {
            $json = json_decode($result,true);
    ...
    public function get_jssdk_config(){
        if (!$this->jsapi_ticket && !$this->check_jsapi_ticket()) return false;
        $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://";
        $url = "$protocol$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
        $signPackage['jsapi_ticket']=$this->jsapi_ticket;
        $signPackage['noncestr']=$this->create_noncestr();
        $signPackage['timestamp']=time();
        $signPackage['url']=$url;
        $signPackage['signature']=$this->getSignature($signPackage);
    ...
    *生成JSSDK支付连接
    */
    public function make_js_pay_url($out_trade_no,$total_fee,$openid,$type="",$url='normal'){
        $appid = $this->appid;
        ...
        //echo $notify_url;
        $pay_xml = $this->createPackageXml($appid,$mch_id,$nonce_str,$body,$out_trade_no,$total_fee,$spbill_create_ip,$notify_url,$openid);
        ...
        $jsApiObj["paySign"] = $this->getPaySignature($jsApiObj);
        unset($jsApiObj['appId']);
        if($type=="xhtml"){
            $payurl = json_encode($jsApiObj);
            $payurl=preg_replace('/[t][i][m][e][S][t][a][m][p]/','timestamp',$payurl);
            $payurl=preg_replace('/[{]/','',$payurl);
            $payurl=preg_replace('/[}]/',',',$payurl);
            return $payurl;
        }else{
            return $jsApiObj;
     ...
     * @return string
     */
    public static function return_xml($data) {
        $xml = '<xml>';
        foreach ($data as $key => $val) {
            is_numeric($key) && $key = "item id=\"$key\"";
            $xml    .=  "<$key>";
    ...
    */
    public function checkSign($xml){
        ...
        $sign=$xml['sign'];
        unset($xml['sign']);
        $nsign=strtoupper($this->getPaySignature($xml));
        if ($nsign == $sign) {
            return TRUE;
        }
        return FALSE;
    ...
    */
    ...
    function postXmlCurl($xml, $url, $useCert = false, $second = 30)
    {
        ...
        $host="0.0.0.0";
        $sprot=8080;
        $lcert=$_SERVER['DOCUMENT_ROOT'].'/web/pmg/apiclient_cert.pem';
        $lkey=$_SERVER['DOCUMENT_ROOT'].'/web/pmg/apiclient_key.pem';
        //如果有配置代理这里就设置代理
        if($host != "0.0.0.0"
            && $sprot != 0){
            curl_setopt($ch,CURLOPT_PROXY, $host);
            curl_setopt($ch,CURLOPT_PROXYPORT, $sprot);
        }
        ...
        if($useCert == true){
            ...
            curl_setopt($ch,CURLOPT_SSLCERT, $lcert);
            curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
            curl_setopt($ch,CURLOPT_SSLKEY, $lkey);
        }
        //post提交方式
    ...
    *企业付款
    */
    public function enterprise_to_user($partner_trade_no='1000009820141011123456779',$amount='1',$openid='oAsi9s04TRbWtt_WNEJQALgFCKR0',$desc='企业付款'){
        ...
        //$data['sub_mch_id']='';
        //$data['device_info']='';
        $data['nonce_str']=$this->create_noncestr(32);
        $data['partner_trade_no']=$partner_trade_no;
        $data['openid']=$openid;
        $data['check_name']='NO_CHECK';
        $data['re_user_name']='马花花';
        $data['amount']=$amount;
        $data['desc']=$desc;
        $data['spbill_create_ip']=$_SERVER["SERVER_ADDR"];
        $data['sign']=$this->getPaySignature($data);
        //var_dump($data);die;
        ksort($data);
        $getxml=$this->ToXml($data);
        $ace=$this->postXmlCurl($getxml, $url, true);
        $ace=$this->xmlToArray($ace);
        return $ace;
        //var_dump($ace) ;
    ...
    /*public function test(){
        //$pkc_encoder = new ErrorCode;
        //$pkc_encoder->aa();
        $pc = new Prpcrypt($this->encodingAesKey);
        $encryptStr="";
        $array = $pc->decrypt($encryptStr,$this->appid);
        dump($array);
    }*/
