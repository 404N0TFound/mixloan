...
class Xuan_mixloan_Member
{
    ...
    */
    function checkAgent($uid) {
        $check = pdo_fetch('SELECT id,msg,effecttime FROM '.tablename("xuan_mixloan_payment")." WHERE uid=:uid ORDER BY id DESC", array(':uid'=>$uid));
        if ($check) {
            if ($check['effecttime'] < time()) {
                return ['code'=>'0','name'=>'过期代理', 'id'=>$check['id']];
            } else {
