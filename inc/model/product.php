        ...
        if (!$list) {
            ...
            foreach ($list as &$row) {
                ...
                if ($row['id']) {
                    ...
                    $pay = pdo_fetchcolumn("SELECT count(1) FROM ".tablename('xuan_mixloan_payment').' WHERE uid=:uid', array(':uid'=>$row['uid']));
                    if ($pay) {
                        $row['pay'] = 1;
                    } else {
                        $row['pay'] = 0;
                ...
                if (!$row['re_bonus']) {
                    $row['re_bonus'] = pdo_fetchcolumn("SELECT re_bonus FROM ".tablename('xuan_mixloan_product_apply').' WHERE uid=:uid AND pid=0', array(':uid'=>$row['uid']));
                    if (!$row['re_bonus']) {
                        $row['re_bonus'] = 0;
                    }
                }
