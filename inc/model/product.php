...
class Xuan_mixloan_Product
{
    ...
    **/
    public function getTopBonus($id){
        ...
        $ret = [];
        $list = pdo_fetchall("SELECT inviter,SUM(relate_money) AS `money`,COUNT(1) AS count FROM ".tablename("xuan_mixloan_product_apply")." WHERE pid={$id}  AND (money!=0 OR inviter!=0) GROUP BY inviter ORDER BY money,count DESC LIMIT 10");
        foreach ($list as $row) {
