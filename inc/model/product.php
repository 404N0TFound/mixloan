...
class Xuan_mixloan_Product
{
    ...
    **/
    public function getSpecialLoan($type) {
        ...
        $sql = "SELECT a.id,a.relate_id,b.name,b.money_high,b.rate,b.rate_type,b.ext_info FROM ".tablename('xuan_mixloan_product')." a LEFT JOIN ".tablename("xuan_mixloan_loan")." b ON a.relate_id=b.id WHERE a.uniacid={$_W['uniacid']} AND a.type=2 AND a.loan_type={$loan_type} AND a.is_show=1 ORDER BY b.id";
        $list = pdo_fetchall($sql);
        $ret = [];
        if (!empty($list)) {
    ...
    **/
    public function getInviteList($params=[]) {
        ...
        //取消时间限制
        $sql = "SELECT {$fields} FROM ".tablename("qrcode_stat")." a LEFT JOIN ".tablename("xuan_mixloan_member")." b ON a.openid=b.openid LEFT JOIN ".tablename("xuan_mixloan_payment")." c ON b.id=c.uid LEFT JOIN ".tablename("xuan_mixloan_product_apply")." d ON b.id=d.uid AND d.pid=0 WHERE a.qrcid=:qrcid AND a.type=1 AND a.uniacid={$_W['uniacid']} GROUP BY a.openid  ORDER BY a.id DESC";
