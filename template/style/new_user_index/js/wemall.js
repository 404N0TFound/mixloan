...
function setProduct (id , name , price , num) {
		$('#'+id).children().css('background-color','#D00A0A');
		var cartMenuN = num;
		$('#totalNum').html( cartMenuN );
		$('#cartN2').html( cartMenuN );
		...
		var wemallId = 'wemall_'+id;
		var html = '<li class="ccbg2" id="'+wemallId+'"><div class="orderdish"><span name="title">'+name+'</span><span class="price" id="v_0" style="display:none;">'+price+'</span><span style="display:none; class="price">元</span></div><div class="orderchange"><a href=javascript:addProductN("'+wemallId+'_'+price+'") class="increase"><b class="ico_increase">加一份</b></a><span class="count" id="num_1_499">' + cartMenuN + '</span><a href=javascript:reduceProductN("'+wemallId+'_'+price+'") class="reduce"><b class="ico_reduce">减一份</b></a></div></li>';
		$('#ullist').html(html);
}
...
function submitInfo() {
	...
	$.ajax({
		type : 'POST',
		url : appurl+'?g=App&m=Index&a=addInfo',
		data : {
			uid : $_GET['uid'],
			userData : $('form').serializeArray()
		},
